...
export const fetchData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        try {
            const response = await axios.post(API_URL + '/admin', param);
            let data = '';
            let _data = response;
            let res = {};
...
export const addData = createAsyncThunk(
    ...
    async (param, thunkAPI) => {
        ...
        try {
            const response = await axios.post(API_URL + '/simpan_admin', param);
            //let data = '';
            let _data = response;
            if (response.status === 200) {
                let dataa = _data.data;
...
export const { addForm, clearError, confirmDel, closeForm } = usersSlice.actions;
